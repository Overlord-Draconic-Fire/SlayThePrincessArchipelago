...
try:
    ...
    _HAS_MP = True
except ImportError:  # Ren'Py runtime may omit multiprocessing
    _HAS_MP = False


# Ren'Py bundles Python 3.9; allow it and just warn instead of aborting.
_force_skip_update = False
if sys.version_info < (3, 11, 0):
    warnings.warn(
        f"Python Version {sys.version_info} is below 3.11; skipping automatic requirements update.",
        RuntimeWarning
    )
    _force_skip_update = True

# don't run update if environment is frozen/compiled or if not the parent process (skip in subprocess)
_skip_update = bool(
    _force_skip_update or
    getattr(sys, "frozen", False) or
    (_HAS_MP and multiprocessing.parent_process()) or
